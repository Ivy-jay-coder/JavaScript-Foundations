...
function mortgageCalculator() {
  const sol = `${name}, your monthly rate is ${monthlyRate}`;
  console.log(sol);
  return sol;
}
mortgageCalculator();
...
Then, add control flow within your function such that IF creditScore is above 740, interest rate drops by 0.05%, if credit score is below 660, interest rate increases by 0.05% and if credit score is anywhere between 660 and 740 interest rate doesn't change.

Hint: To drop an interest rate by .05% you can take monthlyRate and multiply it by 0.95. Similarly, to increase an interest rate by .05% you'd do monthlyRate * 1.05. 
*/
function mortgageCalculator2(P, I, years, creditScore) {
  let monthlyRate =
    (((P * I) / 12) * Math.pow(1 + I / 12, years * 12)) /
    (Math.pow(1 + I / 12, years * 12) - 1);
